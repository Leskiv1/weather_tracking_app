...
class ApiService {
  ...
  Future<Map<String, dynamic>> getForecast() async {
    ...
    if (token == null) {
      return {'success': false, 'error': 'Не авторизовано. Увійдіть в систему.'};
    }
    ...
    if (response.statusCode == 200) {
      final data = jsonDecode(utf8.decode(response.bodyBytes)); // utf8.decode щоб українська мова ("Львів") відображалась коректно
  ...
  Future<Map<String, dynamic>?> getCurrentWeather() async {
    ...
    try {
      final response = await http.get(url); // Тут немає заголовка Authorization!
      if (response.statusCode == 200) {
